...
function build() {
	...
	if [[ "${CI}" == "CONCOURSE" ]]; then
		...
		ls -la
		# shellcheck disable=SC2086
		"${GRADLEW_BIN}" clean build -x test -PnewVersion="${pipelineVersion}" -DREPO_WITH_BINARIES="${REPO_WITH_BINARIES}" -DREPO_WITH_BINARIES_FOR_UPLOAD="${REPO_WITH_BINARIES_FOR_UPLOAD}" --stacktrace ${BUILD_OPTIONS} || (echo "Build failed!!!" && return 1)
	fi
} # }}}
