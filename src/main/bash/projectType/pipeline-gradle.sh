...
function build() {
	...
	if [[ "${CI}" == "CONCOURSE" ]]; then
		...
		# shellcheck disable=SC2086
		"${GRADLEW_BIN}" clean build deploy -x compileTestKotlin -x convertSwagger2markup -x ktlint -PnewVersion="${pipelineVersion}" -DREPO_WITH_BINARIES="${REPO_WITH_BINARIES}" -DREPO_WITH_BINARIES_FOR_UPLOAD="${REPO_WITH_BINARIES_FOR_UPLOAD}" --stacktrace ${BUILD_OPTIONS} || (echo "Build failed!!!" && return 1)
	fi
} # }}}
