...
function build() {
	...
	if [[ "${CI}" == "CONCOURSE" ]]; then
		...
		# shellcheck disable=SC2086
		"${GRADLEW_BIN}" clean build artifactoryPublish -PnewVersion="${pipelineVersion}" -DM2_SETTINGS_REPO_USERNAME="${M2_SETTINGS_REPO_USERNAME}" -DM2_SETTINGS_REPO_PASSWORD="${M2_SETTINGS_REPO_PASSWORD}" -DREPO_WITH_BINARIES="${REPO_WITH_BINARIES}" -DREPO_WITH_BINARIES_FOR_UPLOAD="${REPO_WITH_BINARIES_FOR_UPLOAD}" --stacktrace ${BUILD_OPTIONS} || (echo "Build failed!!!" && return 1)
	fi
} # }}}
